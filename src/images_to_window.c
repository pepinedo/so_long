...
/*   Updated: 2024/07/31 12:58:21 by ppinedo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_image_to_window(data->mlx, data->image_floor,
		P_WIDTH * y, P_HEIGHT * x);
	if (c == '1')
		mlx_image_to_window(data->mlx, data->image_wall,
			P_WIDTH * y, P_HEIGHT * x);
	if (c == 'P')
		mlx_image_to_window(data->mlx, data->image_player,
			P_WIDTH * y, P_HEIGHT * x);
	if (c == 'C')
		mlx_image_to_window(data->mlx, data->image_collectible,
			P_WIDTH * y, P_HEIGHT * x);
	if (c == 'E')
		mlx_image_to_window(data->mlx, data->image_exit,
			P_WIDTH * y, P_HEIGHT * x);
}
...
bool	png_to_image(t_data *data)
{
	...
	if (!data->image_player)
		return (false);
	data->image_collectible = mlx_texture_to_image(data->mlx,
			data->texture_collectible);
